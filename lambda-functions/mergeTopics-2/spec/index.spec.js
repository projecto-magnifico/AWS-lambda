...
describe('formulateInsertionSchema', () => {
    it('converts an array of arrays into', () => {
        expect(formulateInsertionSchema(keywordSets, threadTextAndId2).newThreadSchema).to.eql([0, 1]);
        expect(formulateInsertionSchema(keywordSets, threadTextAndId2).insertionSchema).to.eql([]);
    });
});
